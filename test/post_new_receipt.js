...
describe('Test to post a new receipt', function () {
  ...
  before(function (done) {
    chai.request(server)
      .post(`/app/userID/${userID}/receipt`).send(receiptObj)
  ...
  it('The returned object of the created receipt should have specific values', function() {
    ...
    expect(requestResult.userID).to.equal(receiptObj.userID);
  })

  // after 
  after(function(done){
    chai.request(server)
      .delete(`/app/userID/${userID}/receipt/${requestResult.receiptID}}`)
      .end(function (err, res) {
        requestResult = res.body;
        response = res;
        expect(err).to.be.null;
